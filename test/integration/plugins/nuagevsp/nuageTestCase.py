...
class nuageTestCase(cloudstackTestCase):
    ...
    def dcInternetConnectivityInfo(cls):
        ...
        zone = next(zone for zone in cls.config.zones
                    if zone.name == cls.zone.name)
        if hasattr(zone, "dcInternetConnectivityInfo") and hasattr(zone.dcInternetConnectivityInfo, "available"):
    ...
    def ssh_into_VM(self, vm, public_ip, reconnect=True, negative_test=False):
        ...
        tries = 3 if negative_test else 30
    ...
    def verify_vsd_network(self, domain_id, network, vpc=None,
        ...
        self.debug("Verifying the creation and state of Network - %s in VSD" %
                   network.name)
        if self.isNuageSingleTenancyModeEnabled:
            vsd_enterprise = self.vsd.get_enterprise(
                filter=self.get_externalID_filter(self.root_domain.id))
            self.assertEqual(vsd_enterprise.name, self.root_domain.id,
                             "VSD enterprise should always correspond to "
                             "CloudStack ROOT domain as Nuage VSP single "
                             "tenancy mode is enabled"
                             )
        else:
            vsd_enterprise = self.vsd.get_enterprise(
