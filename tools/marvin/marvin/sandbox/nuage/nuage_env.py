...
class nuage(object):
    def __init__(self, hostname=None, username='csproot', password='csproot', port=8443):
        self.hostname = hostname
        self.username = username
        self.password = password
        self.port = port
        self.apiversion = 'v1_0'
        self.retrycount = '4'
...
def describeResources(config):
    ...
    nuageprovider = provider('NuageVsp')
    nuageprovider.devices.append(nuage(hostname=config.get('nuage', 'host')))

    vpcprovider = provider('VpcVirtualRouter')
    lbprovider = provider('InternalLbVm')

    managementLabel = config.get('cloudstack', 'network.label.management')   
    guestLabel = config.get('cloudstack', 'network.label.guest')   
    hypervisor = config.get('cloudstack', 'network.label.hypervisor')
 
    pn1 = physicalNetwork()
    pn1.name = "Management Network"
    pn1.isolationmethods = ["VLAN"]
    pn1.traffictypes = [trafficType("Management", {hypervisor: managementLabel}),
                        trafficType("Public", {hypervisor: managementLabel}),
                        trafficType("Storage", {hypervisor: managementLabel})]

    pn2 = physicalNetwork()
    pn2.name = "Nuage Network"
    pn2.isolationmethods = ["VSP"]
    pn2.traffictypes = [trafficType("Guest", {hypervisor: guestLabel})]
    pn2.providers.extend([nuageprovider, vpcprovider, lbprovider ])
    ...
    try:
        v.vlan = config.get('cloudstack', 'public.vlan', None)
    except:
        pass
    z.ipranges.append(v)
...
if __name__ == '__main__':
    ...
    parser.add_option('-o', '--output', action='store', default='./nuage.cfg', \
